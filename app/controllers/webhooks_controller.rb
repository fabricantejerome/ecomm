   ...
   skip_forgery_protection
    ...
    def stripe
        ...
        when 'checkout.session.completed'
            ...
            if shipping_details
                address = "#{shipping_details['address']['line1']}, , #{shipping_details['address']['state']}, #{shipping_details['address']['postal_code']}"
            else
                address = ""
            end
            
            order = Order.create!(customer_email: sessoin["customer_details"]["email"], total: session["amount_total"], address: address, fulfilled: false)
            full_session = Stripe::Checkout::Session.retrive({
                id: session.id,
                expand: ['line_items']
            })
            line_items = full_session.line_items
            line_items["data"].each do |item|
                product = Stripe::Product.retrive(item["price"]["product"])
